---
title: "SummarizedExperiment Tutorial-Assignment3Brukt"
...
knitr::opts_knit$set(root.dir = normalizePath("/home1/tefera/490_cluster/analysis_data"))
...
gr <- GRanges(seqnames = Rle("chr1", nrow(rna_genes)),
              ranges = IRanges(start = 1:nrow(rna_genes), width = 1),
              gene_id = rna_genes$gene_id)
...
common_samples <- intersect(colnames(rna_counts), rna_clinical$patient_id)
rna_counts <- rna_counts[, common_samples]
rna_clinical <- rna_clinical[rna_clinical$patient_id %in% common_samples, ]
rna_clinical <- rna_clinical[match(common_samples, rna_clinical$patient_id), ]

rna_se <- SummarizedExperiment(
  assays = list(unstranded = as.matrix(rna_counts)),
  colData = DataFrame(rna_clinical),
  rowRanges = gr
)
...
# Clinical data
rownames(rna_clinical)[1:5]
colnames(rna_clinical)

# Gene data
rownames(rna_genes)[1:5]
colnames(rna_genes)

# what are the rows...?
# Counts data
rownames(rna_counts)[1:5]
colnames(rna_counts)[1:5]
...
rna_clinical$age_at_index[1:15]
...
```{r Exercise 2.3}
colnames_mask <- !(colnames(rna_clinical) %in% c("treatments", "primary_site", "disease_type", "sites_of_involvement"))
...
```{r Exercise 6.1}
write.csv(rna_clinical, "/home1/tefera/analysis_data/brca_rna_clinical_data.csv", row.names = TRUE)
```
...
```{r Exercise 6.2}
write.csv(rna_genes, "/home1/tefera/analysis_data/brca_rna_gene_data.csv", row.names = FALSE)
```
...
```{r Exercise 6.3}
write.csv(rna_counts, "/home1/tefera/analysis_data/brca_rna_count_data.csv", row.names = TRUE)
```
